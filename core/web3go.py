...
class Web3Go:
    ...
    def get_current_date(utc: bool = False, add_days: int = 0, formatted=False):
        if formatted:
            ...
            return datetime.datetime.now(pytz.timezone(my_tz)).strftime("%Y-%m-%d")
        if utc:
            # return (datetime.datetime.now(datetime.UTC) + datetime.timedelta(days=add_days)).strftime("%Y%m%d")
            return (datetime.datetime.now(pytz.timezone(my_tz)) + datetime.timedelta(days=add_days)).strftime("%Y%m%d")
    ...
    async def claim_today(self):
        ...
        # print(start_data, end_data) 20240318 20240319

        url = f"https://reiki.web3go.xyz/api/checkin/points/his?start={start_data}&end={end_data}"
        # print(url) https://reiki.web3go.xyz/api/checkin/points/his?start=20240318&end=20240319
